          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
            {[
              ...
              <div key={kpi.label} style={{ background: "#fff", border: "1px solid #efefef", borderRadius: "8px", padding: "18px 20px" }}>
                ...
                <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", fontWeight: 500, color: "#444", marginBottom: "2px" }}>{kpi.label}</p>
              ...
              return (
                <div key={role} style={{ background: "#fff", border: "1px solid #efefef", borderRadius: "8px", padding: "18px 20px" }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "10px", fontWeight: 600, color: conf.color, textTransform: "uppercase", letterSpacing: "0.08em" }}>{conf.label}</span>
                  </div>
            ...
            {manuscripts.slice(0, 5).map((m) => (
              <div key={m.id} className="flex items-center gap-3 py-3 flex-wrap" style={{ borderBottom: "1px solid #f9f9f9" }}>
                ...
                <div className="flex-1 min-w-0">
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "13px", fontWeight: 500, color: "#333", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.title}</p>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "11px", color: "#bbb" }}>{m.submittedByName} · {m.submittedDate}</p>
                </div>
                {m.timeline[m.timeline.length - 1] && (
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "11px", color: "#bbb", flexShrink: 0 }}>{m.timeline[m.timeline.length - 1].date}</p>
                )}
              </div>
            ))}
          </div>
              ...
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <StatusBadge status={m.status} />
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "10px", color: "#bbb" }}>{m.id.toUpperCase()} · {m.category}</span>
                  </div>
                  <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "15px", fontWeight: 600, color: "#0b0b0b", lineHeight: 1.3, marginBottom: "4px" }}>{m.title}</p>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", color: "#888" }}>{m.submittedByName} — {m.institution} · {m.submittedDate}</p>
                </div>
                <div className="flex gap-2 flex-shrink-0 flex-col items-end">
                  ...
                  {m.status === "published" && m.articleSlug && (
                    <Link to={`/articulo/${m.articleSlug}`} style={{ fontFamily: "'Inter', sans-serif", fontSize: "11px", color: "#3ecf8e", textDecoration: "none" }}>
                      Ver artículo →
                    </Link>
                  )}
                </div>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #f0f0f0", background: "#fafafa" }}>
                  {["Usuario", "Email", "Institución", "Rol", "Manuscritos", "Desde", "Acciones"].map((h) => (
                    <th key={h} style={{ fontFamily: "'Inter', sans-serif", fontSize: "10px", fontWeight: 600, color: "#888", letterSpacing: "0.1em", textTransform: "uppercase", padding: "12px 14px", textAlign: "left" }}>
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={u.id} style={{ borderBottom: "1px solid #f9f9f9" }}>
                    <td style={{ padding: "10px 14px" }}>
                      <div className="flex items-center gap-2">
                        <div style={{ width: "26px", height: "26px", borderRadius: "50%", background: ROLE_CONFIG[u.role as UserRole & string]?.color ?? "#aaa", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", fontWeight: 700, color: "#fff", fontFamily: "'Inter', sans-serif", flexShrink: 0 }}>
                          {u.name.charAt(0)}
                        </div>
                        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "13px", color: "#333", fontWeight: 500, whiteSpace: "nowrap" }}>{u.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: "10px 14px" }}><span style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", color: "#888" }}>{u.email}</span></td>
                    <td style={{ padding: "10px 14px" }}><span style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", color: "#888" }}>{u.institution}</span></td>
                    <td style={{ padding: "10px 14px" }}><RoleBadge role={u.role as UserRole} /></td>
                    <td style={{ padding: "10px 14px" }}><span style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", color: "#888" }}>{u.manuscripts}</span></td>
                    <td style={{ padding: "10px 14px" }}><span style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", color: "#bbb" }}>{u.joined}</span></td>
                    <td style={{ padding: "10px 14px" }}>
                      <div className="relative">
                        <button
                          onClick={() => setUserRoleChanging(userRoleChanging === u.id ? null : u.id)}
                          className="flex items-center gap-1 px-3 py-1.5 rounded"
                          style={{ fontFamily: "'Inter', sans-serif", fontSize: "11px", color: "#555", background: "#f5f5f5", border: "1px solid #e8e8e8", cursor: "pointer" }}
                        >
                          ...
                          <div style={{ position: "absolute", right: 0, top: "32px", background: "#fff", border: "1px solid #e8e8e8", borderRadius: "6px", minWidth: "130px", zIndex: 50, boxShadow: "0 8px 24px rgba(0,0,0,0.1)" }}>
                            {(["investigador", "editor", "jurado", "admin"] as const).filter((r) => r !== u.role).map((role) => (
                              <button
                                ...
                                style={{ width: "100%", textAlign: "left", padding: "9px 14px", fontFamily: "'Inter', sans-serif", fontSize: "12px", color: ROLE_CONFIG[role].color, border: "none", background: "none", cursor: "pointer", display: "block", transition: "background 0.1s" }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div style={{ background: "#fff", border: "1px solid #efefef", borderRadius: "8px", padding: "20px 24px" }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={statusData} dataKey="value" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`} labelLine={false} style={{ fontFamily: "'Inter', sans-serif", fontSize: "10px" }}>
                ...
                {group.fields.map((f) => (
                  <div key={f.label}>
                    ...
                    <input
                      ...
                      style={{ width: "100%", padding: "8px 12px", border: "1px solid #e8e8e8", borderRadius: "4px", fontFamily: "'Inter', sans-serif", fontSize: "13px", color: "#333", outline: "none", background: "#fafafa", boxSizing: "border-box" }}
                    />
                  </div>
                ))}
              </div>
              <button className="mt-4 px-5 py-2 rounded" style={{ background: "#0b0b0b", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: "12px", fontWeight: 500, border: "none", cursor: "pointer" }}>
                Guardar cambios
              </button>
            </div>
          ))}
